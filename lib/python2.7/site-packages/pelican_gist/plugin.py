...
def fetch_gist(gist_id, filename=None):
    ...
    return parse_response_body(body, filename)


def parse_response_body(body, filename=None):
    """Parse the Git API JSON response."""
    bodydict = json.loads(body)['files']
    files = bodydict.keys()

    if filename is not None:
        if filename in files:
            return json.dumps(bodydict[filename]['content'])
        else:
            raise Exception("Unable to find specified " + \
                            "file name: {!s}".format(filename))
